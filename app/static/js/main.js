...
for (var i = 0; i < pbodyElements.length; i++) {
  ...
  pbody.addEventListener('click', function handleClick(event) {
    ...
    if (elementName == "A") {
      ...
      if (event.target.className == "expando" || event.target.className == "unblk" || event.target.className == "report-post" || event.target.className == "delete-post" || event.target.className == "title nsfw-blur False" || event.target.className == "" || event.target.className === undefined) {
        return false;
      }
      //if is link get url and just redirect like a normal link
      var link = event.target.href;
      //check if link is relative or not
      if (link.indexOf("cekni.to") !== -1 && link.indexOf("flair=") === -1 && link.indexOf("/domain/") === -1 && link.indexOf("/u/") === -1) {
        event.preventDefault();
