...
from app.models import Sub, SubSubscriber, SubPost
...
@recount.command(help="Rebuilds all sub's posts counters")
@click.option(
    "--save/--dry-run",
    default=True,
    help="Use --save (the default) to fix the counts, or --dry-run to just print them.",
...
def posts(save):
    ...
    subs = Sub.select(Sub.sid, Sub.name, Sub.posts).order_by(Sub.name)
    for sub in subs:
        count = (
            SubPost.select()
        ...
        print(f"{sub.name:32}{sub.posts:7}{count:8}")
        if save:
            Sub.update(
